...
int toggle_scl(uint16_t sda_pin, uint16_t scl_pin)
{
	int i;
	volatile unsigned int *sda = (volatile unsigned int *)GPIO_IO(sda_pin);

	if (*sda)
		return 0;

	// set SCL as output gpio
	(*(volatile unsigned int *)GPIO_CFG(scl_pin)) = 0x203;
	udelay(TOGGLE_DELAY_US);

	// toggle SCL until SDA is released or we've tried max times
	for (i = 0; i < TOGGLE_MAX_RETRIES; i++) {
		(*(volatile unsigned int *)GPIO_IO(scl_pin)) = 0x2;
		udelay(TOGGLE_DELAY_US);
		(*(volatile unsigned int *)GPIO_IO(scl_pin)) = 0x0;
		udelay(TOGGLE_DELAY_US);
